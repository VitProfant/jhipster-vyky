  ...
  return (
    <div>
      <h2 id="message-heading" data-cy="MessageHeading">
        <Translate contentKey="jhipsterVykyApp.message.home.title">Messages</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterVykyApp.message.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/message/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterVykyApp.message.home.createLabel">Create new Message</Translate>
          </Link>
        </div>
      </h2>
        ...
        {messageList && messageList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="jhipsterVykyApp.message.id">Id</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th className="hand" onClick={sort('deletedByAuthor')}>
                  <Translate contentKey="jhipsterVykyApp.message.deletedByAuthor">Deleted By Author</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('deletedByAuthor')} />
                </th>
                <th>
                  <Translate contentKey="jhipsterVykyApp.message.parent">Parent</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="jhipsterVykyApp.message.author">Author</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {messageList.map((message, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{message.deletedAt ? <TextFormat type="date" value={message.deletedAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{message.deletedByAuthor ? 'true' : 'false'}</td>
                  <td>{message.parent ? <Link to={`/message/${message.parent.id}`}>{message.parent.id}</Link> : ''}</td>
                  <td>{message.author ? <Link to={`/vyky-user/${message.author.id}`}>{message.author.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterVykyApp.message.home.notFound">No Messages found</Translate>
            </div>
          )
        )}
      </div>
